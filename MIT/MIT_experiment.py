...
def run_single_iteration(req_min: int, input_tokens: int, output_tokens: int) -> None:
    ...
    # Store experiment artifacts into MIT/data/<INPUT>_<OUTPUT>/<DATE>/ using MST/store_results.py
    try:
        root_dir = Path(__file__).resolve().parent.parent  # .../MoST
        store_script = root_dir / 'MST' / 'store_results.py'
        parent_dir = Path(__file__).resolve().parent / 'data' / f"{input_tokens}_{output_tokens}"

        # Pull optional metadata from env if available
        model = os.environ.get('MODEL', '')
        gpus = os.environ.get('GPUS', '')
        cpus = os.environ.get('CPUS', '')
        node = os.environ.get('NODE', '')
        stage = f"MIT_REQMIN_{req_min}"

        # Build command with quoted paths for Windows compatibility
        cmd = (
            f"python -u \"{store_script}\" "
            f"\"{model}\" \"{gpus}\" \"{cpus}\" \"{node}\" "
            f"\"{stage}\" \"{parent_dir}\""
        )
        run_cmd(cmd, wait=True)
    except Exception as e:
        print(f"Warning: storing results failed: {e}")
